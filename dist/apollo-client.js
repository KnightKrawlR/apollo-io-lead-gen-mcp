...
export class ApolloClient {
     ...
     */
    async peopleEnrichment(query) {
        try {
            const url = `${this.baseUrl}/people/match`;
            console.log('People Enrichment URL:', url);
            console.log('People Enrichment Query:', query);
     ...
     */
    async organizationEnrichment(query) {
        try {
            const url = `${this.baseUrl}/organizations/enrich`;
            console.log('Organization Enrichment URL:', url);
            console.log('Organization Enrichment Query:', query);
    ...
    async peopleSearch(query) {
        try {
            ...
            console.log('People Search URL:', url);
            console.log('People Search Query:', JSON.stringify(query, null, 2));
            const response = await this.axiosInstance.post(url, query);
            if (response.status === 200) {
                return response.data;
            }
     ...
     */
    async organizationSearch(query) {
        try {
            const url = `${this.baseUrl}/mixed_companies/search`;
            console.log('Organization Search URL:', url);
            console.log('Organization Search Query:', JSON.stringify(query, null, 2));
            const response = await this.axiosInstance.post(url, query);
            if (response.status === 200) {
                return response.data;
            }
     ...
     */
    async organizationJobPostings(organizationId) {
        try {
            const url = `${this.baseUrl}/organizations/${organizationId}/job_postings`;
            console.log('Organization Job Postings URL:', url);
    ...
    async getPersonEmail(apolloId) {
        try {
            ...
            console.log('Get Person Email URL:', baseUrl);
            console.log('Get Person Email Payload:', payload);
            ...
            const emails = (response?.data?.contacts ?? []).map((item) => item.email);
            return emails;
        }
        catch (error) {
     ...
     */
    async employeesOfCompany(query) {
        try {
            ...
            console.log('Employees of Company - Company Search Payload:', companySearchPayload);
            const mixedCompaniesResponse = await axios.post('https://api.apollo.io/api/v1/mixed_companies/search', companySearchPayload, {
                headers: {
                    'Content-Type': 'application/json',
                    'X-Api-Key': this.apiKey
                }
            ...
            const companyObjs = organizations.filter((item) => {
            ...
            if (query.person_seniorities) {
                peopleSearchPayload.person_seniorities = (query.person_seniorities ?? '').split(',').map((item) => item.trim());
            }
            if (query.contact_email_status) {
                peopleSearchPayload.contact_email_status = (query.contact_email_status ?? '').split(',').map((item) => item.trim());
            }
            console.log('Employees of Company - People Search Payload:', peopleSearchPayload);
            const peopleResponse = await axios.post('https://api.apollo.io/api/v1/mixed_people/search', peopleSearchPayload, {
                headers: {
                    'Content-Type': 'application/json',
                    'X-Api-Key': this.apiKey
                }
            ...
            return peopleResponse.data.people || peopleResponse.data.contacts || [];
        }
        catch (error) {
