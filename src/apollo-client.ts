  ...
  async employeesOfCompany(query: EmployeesOfCompanyQuery): Promise<any> {
    try {
      ...
      if (query.person_seniorities) {
        peopleSearchPayload.person_seniorities = (query.person_seniorities ?? '').split(',').map((item: string) => item.trim());
      }
      
      if (query.contact_email_status) {
        peopleSearchPayload.contact_email_status = (query.contact_email_status ?? '').split(',').map((item: string) => item.trim());
      }
